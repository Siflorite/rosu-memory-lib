...
use rosu_memory_lib::reader::{
    beatmap::{
        common::{BeatmapInfo, BeatmapLocation, BeatmapMetadata, BeatmapStats, BeatmapStatus, BeatmapStarRating},
        stable::file as rust_file,
    },
    structs::State,
};
use rosu_memory_lib::common::GameMode;
use crate::reader::beatmap::common::{PyBeatmapInfo, PyBeatmapLocation, PyBeatmapMetadata, PyBeatmapStats, PyBeatmapStatus};
use crate::reader::common::PyGameMode;

#[pyclass(name = "BeatmapStarRating")]
#[derive(Clone)]
...
pub fn get_beatmap_stats(process: &Process, state: &mut State) -> PyResult<PyBeatmapStats> {
    let stats = rust_file::get_beatmap_stats(process, state)?;
    Ok(PyBeatmapStats {
        ar: stats.ar,
        od: stats.od,
        cs: stats.cs,
...
pub fn get_beatmap_info(process: &Process, state: &mut State) -> PyResult<PyBeatmapInfo> {
    let info = rust_file::get_beatmap_info(process, state)?;
    Ok(PyBeatmapInfo {
