...
pub fn get_folder(p: &Process, state: &mut State) -> eyre::Result<String> {
    Ok(read_from_beatmap_ptr_string(p,state,BEATMAP_OFFSET.location.folder)?)

}
pub fn get_filename(p: &Process, state: &mut State) -> eyre::Result<String> {
    Ok(read_from_beatmap_ptr_string(p,state,BEATMAP_OFFSET.location.filename)?)

}

pub fn get_audio(p: &Process, state: &mut State) -> eyre::Result<String> {
    Ok(read_from_beatmap_ptr_string(p,state,BEATMAP_OFFSET.location.audio)?)
}

pub fn get_cover(p: &Process, state: &mut State) -> eyre::Result<String> {
    Ok(read_from_beatmap_ptr_string(p,state,BEATMAP_OFFSET.location.cover)?)
}
